...
extension Metadata {
    public init(_ data: WIN32_FIND_DATAW) {
        ...
        if data.dwFileAttributes & UInt32(bitPattern: FILE_ATTRIBUTE_REPARSE_POINT) != 0 && data.dwReserved0 & 0x2000_0000 != 0 {
            fileType = WindowsFileType.symlink
        } else if data.dwFileAttributes & UInt32(bitPattern: FILE_ATTRIBUTE_DIRECTORY) != 0 {
